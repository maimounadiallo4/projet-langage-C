...
int main() {
    ...
    while (fgets(ligne, sizeof(ligne), fichier_tests)) {
        ...
        if (strcmp(attendu, "ERREUR") == 0) {
            if (strstr(sortie_programme, "erronee") || strstr(sortie_programme, "Erreur")) {
                succes = 1;
                printf(" [ " VERT "✔" RESET " ] %-32s " BLEU "→" RESET " " VERT "Erreur détectée" RESET "\n", expression_str);
            } else {
                printf(" [ " ROUGE "✘" RESET " ] %-32s " BLEU "→" RESET " " ROUGE "Erreur ATTENDUE" RESET "\n", expression_str);
            }
        } 
        /* Cas 2 : On attend une valeur numérique précise */
            ...
            if (strstr(sortie_programme, chaine_recherche)) {
                succes = 1;
                printf(" [ " VERT "✔" RESET " ] %-32s " BLEU "→" RESET " " VERT "%-10s" RESET "\n", expression_str, attendu);
            } else {
                printf(" [ " ROUGE "✘" RESET " ] %-32s " BLEU "→" RESET " " ROUGE "Attendu: %-10s" RESET "\n", expression_str, attendu);
            }
        }

        if (succes) reussis++; else echecs++;
    }
    ...
    if (echecs == 0) {
        printf(CYAN "──────────────────────────────────────────────────────────\n" RESET);
        printf(" " VERT GRAS "BRAVO !" RESET " Tous les tests ont été validés avec succès.\n");
        printf(" Score final : " GRAS VERT "%d" RESET "/" GRAS "%d" RESET " [" VERT "100%%" RESET "]\n", reussis, total_tests);
        printf(CYAN "──────────────────────────────────────────────────────────\n" RESET);
    } else {
        printf(ROUGE "──────────────────────────────────────────────────────────\n" RESET);
        printf(" " ROUGE GRAS "ATTENTION !" RESET " Certains tests ont échoué.\n");
        printf(" Réussis : " VERT "%d" RESET " | Échecs : " ROUGE "%d" RESET " | Total : %d\n", reussis, echecs, total_tests);
        printf(ROUGE "──────────────────────────────────────────────────────────\n" RESET);
    }
